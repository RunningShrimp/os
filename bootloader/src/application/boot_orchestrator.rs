...
use crate::{
    domain::{
        events::DomainEventPublisher, hardware_detection::HardwareDetectionService,
        repositories::BootConfigRepository, *,
    },
    infrastructure::{BootDIContainer, DIContainer},
...
impl BootApplicationService {
    ...
    pub fn new(di_container: Box<DIContainer>) -> BootResult<Self> {
        ...
        let event_publisher = di_container
            ...
            .map_err(|e| {
                BootError::ServiceResolutionFailed(format!(
                    "Failed to resolve DomainEventPublisher: {}",
                    e
                ))
            })?;
        ...
        let hardware_detection = di_container
            ...
            .map_err(|e| {
                BootError::ServiceResolutionFailed(format!(
                    "Failed to resolve HardwareDetectionService: {}",
                    e
                ))
            })?;
        ...
        let config_repo = di_container
            ...
            .map_err(|e| {
                BootError::ServiceResolutionFailed(format!(
                    "Failed to resolve BootConfigRepository: {}",
                    e
                ))
            })?;
    ...
    pub fn with_default_container(protocol_type: BootProtocolType) -> BootResult<Self> {
        ...
        inner_container.auto_discover_services().map_err(|_| {
            BootError::ProtocolInitializationFailed("Service discovery failed".to_string())
        })?;
    ...
    pub fn from_config(config_str: &str, format: &str) -> BootResult<Self> {
        ...
        let di_container = match format {
            "toml" => DIContainer::from_toml_config(config_str).map_err(|_| {
                BootError::ConfigurationError(String::from("TOML configuration error"))
            })?,
            "json" => DIContainer::from_json_config(config_str).map_err(|_| {
                BootError::ConfigurationError(String::from("JSON configuration error"))
            })?,
            _ => {
                return Err(BootError::ConfigurationError(String::from(
    ...
    pub fn boot_system(&mut self, cmdline: Option<&str>) -> BootResult<BootInfo> {
        ...
        let kernel_info = KernelInfo::new(0x100000, 0x500000, 0x100000)
            .map_err(|e| BootError::ValidationError(e.to_string()))?;
        boot_info
        ...
        boot_info.validate().map_err(|_error| {
    ...
    fn load_boot_configuration(&self, cmdline: Option<&str>) -> BootResult<BootConfig> {
        if let Some(cmd) = cmdline {
            self.config_repo.create_from_cmdline(cmd).map_err(|e| {
                BootError::ConfigurationError(format!(
                    "Failed to create config from cmdline: {}",
                    e
                ))
            })
        } else {
            self.config_repo.load_default().map_err(|e| {
                BootError::ConfigurationError(format!("Failed to load default config: {}", e))
            })
        }
    }
    ...
    fn validate_prerequisites(
        &self,
        config: &BootConfig,
        hw_info: &crate::domain::boot_services::HardwareInfo,
    ) -> BootResult<()> {
        BootValidator::validate_prerequisites(config, hw_info).map_err(|_errors| {
    ...
    fn initialize_graphics(&mut self, config: &BootConfig) -> BootResult<()> {
        if let Some(mode) = config.graphics_mode {
            ...
            if let Ok(mut graphics_backend) = self.di_container.resolve::<Box<
                dyn crate::infrastructure::graphics_backend::GraphicsBackend,
            >>("GraphicsBackend")
            {
                ...
                if let Some(fb_info) = graphics_backend.get_framebuffer_info() {
                    ...
                    let event = Box::new(
                        events::GraphicsInitializedEvent::new(
                            mode.width,
                            mode.height,
                            fb_info.address, // Real address from backend
                            0,               /* Timestamp (would come from time service in real
                                              * implementation) */
                        )
                        .with_bpp(fb_info.bpp as u8)
                        .with_backend("VBE"),
                    ); // Would detect actual backend type in real implementation
                    self.event_publisher.publish(event).map_err(|e| {
                        BootError::EventError(format!("Failed to publish graphics event: {}", e))
                    })?;
                }
            } else {
                return Err(BootError::ServiceResolutionFailed(
                    "Graphics backend not available".to_string(),
                ));
            }
        }
        Ok(())
    ...
    fn create_boot_info(&self, config: &BootConfig) -> BootResult<BootInfo> {
        let boot_info = BootInfo::from_config(config, self.get_protocol_type()).map_err(|e| {
            BootError::ValidationError(format!("Failed to create boot info: {}", e))
        })?;
    ...
    fn publish_boot_ready_event(&mut self, boot_info: &BootInfo) -> BootResult<()> {
        ...
        self.event_publisher.publish(event).map_err(|e| {
            BootError::EventError(format!("Failed to publish boot ready event: {}", e))
        })
    }
