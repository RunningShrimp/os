...
use crate::domain::{
    ...
    transactions::{
        TransactionError, TransactionLog, TransactionLogEntry, TransactionLogStats,
        TransactionLogType, TransactionOperation,
    },
};
...
impl TransactionLog for MemoryTransactionLog {
    ...
    fn get_log_stats(&self) -> Result<TransactionLogStats, TransactionError> {
        ...
        // 更新实时统计信息
        {
            ...
            stats.total_entries =
                self.operation_counter.load(Ordering::SeqCst) as usize + history.len();
...
mod tests {
    ...
    fn test_clear_transaction_log() {
        ...
        assert_eq!(
            log.get_transaction_operations(transaction_id)
                .unwrap()
                .len(),
            1
        );
