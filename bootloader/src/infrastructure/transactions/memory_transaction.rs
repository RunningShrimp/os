...
use crate::domain::{
    ...
    transactions::{
        Transaction, TransactionError, TransactionLog, TransactionOperation, TransactionStatus,
    },
};
    ...
    fn validate_transaction_state(&self) -> Result<(), TransactionError> {
        ...
        match *status {
            ...
            TransactionStatus::Committed => {
                Err(TransactionError::TransactionAlreadyCommitted(self.id))
            },
            TransactionStatus::RolledBack => {
                Err(TransactionError::TransactionAlreadyRolledBack(self.id))
            },
            TransactionStatus::Timeout => Err(TransactionError::TransactionTimeout(self.id)),
            TransactionStatus::Failed => {
                Err(TransactionError::OperationError("Transaction has failed"))
            },
        }
    }
...
mod tests {
    ...
    fn test_transaction_pool() {
        ...
        let transaction_log = Arc::new(MemoryTransactionLog::new());
        let transaction =
            MemoryTransaction::new(TransactionId::new(1), transaction_log, None).unwrap();
