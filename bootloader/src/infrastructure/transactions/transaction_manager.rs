...
use crate::domain::{
    repositories::{IdGenerator, TransactionId},
    transactions::{
        Transaction, TransactionError, TransactionLog, TransactionManager, TransactionStats,
        TransactionStatus,
    },
};
...
impl TransactionManager for MemoryTransactionManager {
    ...
    fn begin_transaction_with_timeout(
        ...
        // 创建事务
        let transaction =
            MemoryTransaction::new(id, self.transaction_log.clone(), Some(timeout_ms))?;
        let transaction_id = transaction.id();

        // 添加到事务池
        self.transaction_pool
            .add_transaction(Box::new(transaction))?;
...
mod tests {
    ...
    use super::*;
    use crate::domain::{repositories::DefaultIdGenerator, transactions::MemoryTransactionLog};
