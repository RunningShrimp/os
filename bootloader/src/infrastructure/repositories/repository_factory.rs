...
use crate::{
    domain::{
        ...
        boot_config::{GraphicsInfo, KernelInfo, MemoryRegion},
        boot_info::BootInfo,
        repositories::{
            BasicRepositoryQuery, BootInfoRepository, EntityId, GraphicsInfoRepository,
            IdGenerator, KernelInfoRepository, MemoryRegionRepository, Repository, RepositoryError,
        },
        serialization::{BinarySerializer, JsonSerializer},
        transactions::TransactionManager,
    },
    infrastructure::serialization::SerializerRegistry,
};
...
pub trait RepositoryFactory: Send + Sync {
    ...
    /// 创建MemoryRegion仓储
    fn create_memory_region_repository(
        &self,
    ) -> Result<Box<dyn MemoryRegionRepository>, FactoryError>;

    /// 创建KernelInfo仓储
    fn create_kernel_info_repository(&self) -> Result<Box<dyn KernelInfoRepository>, FactoryError>;

    /// 创建GraphicsInfo仓储
    fn create_graphics_info_repository(
        &self,
    ) -> Result<Box<dyn GraphicsInfoRepository>, FactoryError>;
...
impl FactoryStats {
    ...
    pub fn total_repositories(&self) -> usize {
        self.boot_info_repositories
            + self.memory_region_repositories
            + self.kernel_info_repositories
            + self.graphics_info_repositories
    }
}
...
impl DefaultRepositoryFactory {
    ...
    /// 创建内存仓储实例
    fn create_memory_repository<T: AggregateRoot>(
        &self,
    ) -> Result<MemoryRepository<T>, FactoryError> {
        // 检查配置是否启用缓存
...
impl<T: AggregateRoot> SpecificRepositoryWrapper<T> {
    ...
    pub fn new(repository: Box<dyn Repository<T>>, repository_type: &'static str) -> Self {
        Self { inner: repository, repository_type }
    }
...
impl<T: AggregateRoot> Repository<T> for SpecificRepositoryWrapper<T> {
    ...
    // fn find_with_pagination(&self, page: usize, size: usize) ->
    // Result<crate::domain::repositories::Page<T>, RepositoryError> {     self.inner.
    // find_with_pagination(page, size) }
