...
use crate::domain::{
    ...
    repositories::{
        BasicRepositoryQuery, EntityId, IdGenerator, Page, Repository, RepositoryError,
        RepositoryQuery,
    },
    serialization::{SerializationContext, TypedSerializer},
    transactions::{TransactionManager, TransactionOperation},
};
...
impl<T: AggregateRoot> BasicRepositoryQuery<T> for MemoryRepository<T> {
    ...
    fn delete_batch(&self, ids: Vec<EntityId>) -> Result<(), RepositoryError> {
        ...
        // 批量删除实体
        {
            ...
            self.entity_count
                .fetch_sub(ids_len as u64, Ordering::SeqCst);
        }
...
mod tests {
    ...
    use super::*;
    use crate::domain::{
        repositories::{DefaultIdGenerator, SimpleSerializationService},
        serialization::{BinarySerializer, SerializerRegistry},
        transactions::{MemoryTransactionLog, MemoryTransactionManager},
    };
    ...
    fn create_test_repository() -> MemoryRepository<TestEntity> {
        ...
        let transaction_manager =
            Arc::new(MemoryTransactionManager::new(transaction_log, id_generator.clone()));

        MemoryRepository::new(transaction_manager, serializer, id_generator, serializer_impl)
    }
    ...
    fn test_find_by_predicate() {
        ...
        let results = repo
            .find_by_predicate(|e| e.name.starts_with("Test"))
            .unwrap();
