...
use crate::domain::{
    ...
    serialization::{
        DeserializationResult, SerializationContext, SerializationFormat, SerializationResult,
...
impl Serializer for SimpleSerializer {
    fn supported_formats(&self) -> Vec<SerializationFormat> {
        vec![SerializationFormat::Binary, SerializationFormat::Json]
    }
...
impl<T: AggregateRoot> TypedSerializer<T> for SimpleSerializer {
    fn serialize(
        ...
        match context.format {
            ...
            SerializationFormat::Custom(name) => Err(RepositoryError::SerializationError(format!(
...
impl SimpleSerializer {
  ...
  "metadata": {{
    ...
    fn deserialize_binary<T: AggregateRoot>(
        ...
        let _entity_str = core::str::from_utf8(entity_data).map_err(|_| {
...
mod tests {
    ...
    use crate::domain::{
        ...
        repositories::{AggregateRoot, DefaultIdGenerator, EntityId},
    };
    ...
    fn test_json_serialization() {
        let serializer = SimpleSerializer::new();
        let entity = TestEntity::new("Test".to_string());
        let context = SerializationContext::json()
            .with_version(true)
            .with_metadata(true);
