...
use crate::domain::{
    RepositoryError,
    serialization::{SerializationFormat, Serializer},
};
...
impl SerializerRegistry {
        ...
        // 检查名称是否已存在
        {
            ...
            if serializers.contains_key(&name) {
                return Err(RepositoryError::SerializationError(format!(
                    "Serializer with name '{}' already registered",
                    name
                )));
            }
        }
        ...
        // 注册格式映射
        {
            ...
            for format in formats {
                if format_map.contains_key(&format) {
                    return Err(RepositoryError::SerializationError(format!(
                        "Format '{:?}' already registered by serializer '{}'",
                        format,
                        format_map.get(&format).unwrap()
                    )));
                }
    ...
    pub fn unregister(&mut self, name: &str) -> Result<(), RepositoryError> {
        ...
        if let Some(serializer) = serializer {
            ...
            Err(RepositoryError::SerializationError(format!(
                "Serializer with name '{}' not found",
                name
            )))
        }
    }
...
mod tests {
    ...
    fn test_get_serializer_info() {
        ...
        assert!(
            serializer_info
                .supported_formats
                .contains(&SerializationFormat::Binary)
        );
    }
