...
use super::{aggregate_root::AggregateRoot, repositories::RepositoryError};
...
impl<T: AggregateRoot> DeserializationResult<T> {
    ...
    pub fn new(entity: T, format: SerializationFormat) -> Self {
        Self { entity, format, version: None, metadata: Vec::new() }
    }
...
impl<T: AggregateRoot> TypedSerializer<T> for BinarySerializer {
    fn serialize(
        ...
        if context.include_metadata {
            result = result.with_metadata("entity_type".to_string(), entity_type.to_string());
    ...
    fn deserialize(
        ...
        let entity_type = core::str::from_utf8(&data[offset..offset + type_len]).map_err(|_| {
        ...
        if offset + 8 > data.len() {
            return Err(RepositoryError::SerializationError("Insufficient data for ID".to_string()));
        }
        ...
        let entity_str = core::str::from_utf8(entity_data).map_err(|_| {
            RepositoryError::SerializationError("Invalid entity data encoding".to_string())
        })?;

        log::debug!(
            "Deserialized entity data: {} bytes, content preview: {}",
            data_len,
            if entity_str.len() > 50 {
                &entity_str[..50]
            } else {
                entity_str
            }
...
impl<T: AggregateRoot> TypedSerializer<T> for JsonSerializer {
  ...
  "data": {:?}
        ...
        let mut result =
            SerializationResult::new(json_data.into_bytes(), SerializationFormat::Json);
        ...
        // 添加元数据
        if context.include_metadata {
            result = result.with_metadata("entity_type".to_string(), T::entity_type().to_string());
        }
    ...
    fn deserialize(
        ...
        // JSON反序列化的简化实现
        Err(RepositoryError::SerializationError("JSON deserialization not implemented".to_string()))
    }
}
...
impl SerializerRegistry {
    ...
    pub fn get_serializer(&self, format: SerializationFormat) -> Option<&dyn Serializer> {
        self.serializers
            .iter()
            .find(|s| s.supports_format(format))
            .map(|s| s.as_ref())
    }
...
mod tests {
    use super::{
        super::{
            aggregate_root::VersionedAggregateRoot,
            repositories::{AggregateRoot, EntityId},
        },
        *,
    };
    ...
    impl TestEntity {
        fn new(name: String) -> Self {
            Self { id: EntityId::new(0), name, version: 1 }
        }
    }
