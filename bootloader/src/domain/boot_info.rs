...
use crate::{
    domain::{
        AggregateRoot,
        boot_config::{BootPhase, GraphicsInfo, KernelInfo, MemoryRegion},
...
impl fmt::Display for BootInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "  Available Memory: {} MB", self.total_available_memory() / (1024 * 1024))?;
    ...
    fn validate(&self) -> Result<(), RepositoryError> {
        ...
        if self.current_phase == BootPhase::Initialization {
            return Err(RepositoryError::InvalidEntity(
                "Boot phase cannot be in initialization state",
            ));
        }
        ...
        if let Some(ref kernel) = self.kernel_info {
            ...
            for region in &self.memory_regions {
                ...
                {
                    return Err(RepositoryError::InvalidEntity(
                        "Kernel memory conflicts with reserved memory region",
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn test_boot_info_kernel_memory_conflict() {
        ...
        let reserved_region =
            MemoryRegion::new(0x100000, 0x200000, MemoryRegionType::Reserved).unwrap();
