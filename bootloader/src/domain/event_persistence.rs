...
use crate::domain::events::{DomainEvent, DomainEventSubscriber, EventSeverity};
...
pub trait EventReplayer {
    ...
    fn replay_events(
        &self,
        filter: Option<&dyn Fn(&dyn DomainEvent) -> bool>,
    ) -> Vec<&Box<dyn DomainEvent>>;
    ...
    fn replay_events_by_time_range(
        &self,
        start_time: u64,
        end_time: u64,
    ) -> Vec<&Box<dyn DomainEvent>>;
...
impl PersistentEventStore {
    ...
    fn rebuild_indexes(&mut self) {
        ...
        let event_info: Vec<(usize, &str, EventSeverity, u64)> = self
            .memory_store
            .iter()
            .enumerate()
            .map(|(index, event)| (index, event.event_type(), event.severity(), event.timestamp()))
            .collect();
    ...
    pub fn get_latest_events_by_type(
        &self,
        event_type: &str,
        count: usize,
    ) -> Vec<Box<dyn DomainEvent>> {
        ...
        if let Some(indices) = self.event_type_index.get(event_type) {
            ...
            let start_index = if indices.len() > count {
                indices.len() - count
            } else {
                0
            };
    ...
    fn store_event(&mut self, event: Box<dyn DomainEvent>) -> Result<(), &'static str> {
        ...
        while self.memory_store.len() > self.max_events {
            ...
            let removed_event = self
                .memory_store
                .pop_front()
                .ok_or("Failed to remove event")?;

            // Log the removed event information
            log::debug!(
                "Removed event: type={}, id={}",
                removed_event.event_type(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ReplayFilter::TimeRange(start, end) => {
                write!(f, "ReplayFilter::TimeRange({:?}, {:?})", start, end)
            },
...
impl DiagnosticEventReplayer {
    ...
    pub fn new(event_store: Box<dyn EventStore>) -> Self {
        Self { event_store, replay_position: 0, replay_filters: Vec::new() }
    }
...
mod tests {
    ...
    fn test_diagnostic_replayer() {
        ...
        assert!(
            replayer
                .replay_to_subscriber(&test_subscriber, Some(10))
                .is_ok()
        );
