...
| 任务       | 文件                                                   | 具体行动                                      | 优先级  |
| -------- | ---------------------------------------------------- | ----------------------------------------- | ---- |
| 实现无锁队列   | `kernel/src/subsystems/scheduler/unified.rs`         | 用无锁MPSC队列替代Mutex保护的per-CPU ready\_queue   | 🔴 高 |
| 优化工作窃取   | `kernel/src/subsystems/scheduler/unified.rs:424-433` | 实现随机化工作窃取而非顺序遍历，添加窃取概率控制                  | 🔴 高 |
| 批量操作接口   | `kernel/src/subsystems/scheduler/unified.rs`         | 添加batch\_enqueue/batch\_dequeue方法，减少锁获取次数 | 🟡 中 |
...
| 实现完整SELinux引擎 | 完成策略引擎、规则加载、决策逻辑，集成到permission\_check | 🟡 中 |
...
| 服务注册表增强  | 完善service\_registry支持用户空间服务发现      | 🟡 中 |
...
* [x] P1.1.1 合并mm和subsystems/mm模块，移除重复类型定义

* [x] P1.1.2 统一进程管理模块单一入口

* [x] P1.1.3 解决VFS与FS循环依赖

* [x] P1.2.1 合并ext4.rs与ext4\_enhanced.rs

* [x] P1.2.2 合并tcp.rs与tcp\_enhanced.rs

* [x] P1.2.3 合并signal.rs与signal\_enhanced.rs

* [x] P1.3.1 修复ASLR使用RDRAND替代计数器

* [x] P1.3.2 实现TPM真实MMIO访问

* [x] P1.3.3 实现KPTI页表隔离

* [x] P1.3.4 添加Retpoline编译器屏障

* [x] P1.4.1 实现无锁per-CPU队列

* [x] P1.4.2 优化工作窃取算法

* [x] P1.5.1 移除per-CPU分配器的Mutex

* [x] P1.6.1 Socket使用HashMap替代Vec

* [x] P1.6.2 实现端口位图分配器

* [x] P1.7.1 拆分posix/mod.rs为多个模块
...
* [ ] P2.2.1 创建统一memory\_layout.rs

* [ ] P2.2.2 重构循环依赖

* [ ] P2.2.3 文档化混合架构边界

* [ ] P2.2.4 增强service\_registry
