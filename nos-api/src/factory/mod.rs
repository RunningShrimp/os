...
pub trait MemoryManagerFactoryObj: Send + Sync {
    fn create_memory_manager_box(&self)
    -> Result<Box<dyn crate::memory::interface::MemoryManager>>;
}

pub trait ProcessManagerFactoryObj: Send + Sync {
    fn create_process_manager_box(
        &self,
    ) -> Result<Box<dyn crate::process::interface::ProcessManager>>;
}

pub trait SyscallDispatcherFactoryObj: Send + Sync {
    fn create_syscall_dispatcher_box(
        &self,
    ) -> Result<Box<dyn crate::syscall::interface::SyscallDispatcher>>;
}
...
impl FactoryRegistry {
    ...
    pub fn register_service_factory(
        &mut self,
        name: &'static str,
        factory: Box<dyn ServiceFactoryObj>,
    ) {
        self.service_factories.insert(name, factory);
    }

    /// Registers a memory manager factory
    pub fn register_memory_manager_factory(
        &mut self,
        name: &'static str,
        factory: Box<dyn MemoryManagerFactoryObj>,
    ) {
        self.memory_manager_factories.insert(name, factory);
    }

    /// Registers a process manager factory
    pub fn register_process_manager_factory(
        &mut self,
        name: &'static str,
        factory: Box<dyn ProcessManagerFactoryObj>,
    ) {
        self.process_manager_factories.insert(name, factory);
    }

    /// Registers a syscall dispatcher factory
    pub fn register_syscall_dispatcher_factory(
        &mut self,
        name: &'static str,
        factory: Box<dyn SyscallDispatcherFactoryObj>,
    ) {
    ...
    pub fn create_memory_manager(
        ...
        if let Some(factory) = self.memory_manager_factories.get(&name) {
            ...
            let error = crate::error::Error::NotFound(format!(
                "Memory manager factory '{}' not found",
                name
            ));
            Err(error)
        }
    ...
    pub fn create_process_manager(
        ...
        if let Some(factory) = self.process_manager_factories.get(&name) {
            ...
            let error = crate::error::Error::NotFound(format!(
                "Process manager factory '{}' not found",
                name
            ));
            Err(error)
        }
    ...
    pub fn create_syscall_dispatcher(
        &self,
        name: &str,
    ) -> Result<Box<dyn crate::syscall::interface::SyscallDispatcher>> {
        if let Some(factory) = self.syscall_dispatcher_factories.get(&name) {
            ...
            let error = crate::error::Error::NotFound(format!(
                "Syscall dispatcher factory '{}' not found",
                name
            ));
            Err(error)
        }
