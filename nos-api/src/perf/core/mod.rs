...
/// 获取当前时间戳（纳秒）
/// Note: This function needs to be implemented by the using crate
...
impl<T: Clone> CacheEntry<T> {
    ...
    /// Clone implementation that deep copies the data but resets the atomic counters
    pub fn clone_new(&self) -> CacheEntry<T> {
        let _now = get_current_timestamp_ns();
        CacheEntry::new(self.data.clone(), self.ttl_ns)
    }
}
