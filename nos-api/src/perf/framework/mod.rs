...
//!
//! This module provides a unified performance optimization framework, including:
...
impl OptimizationStrategy for FastPathOptimization {
    ...
    fn apply(&self, _context: &mut OptimizationContext) -> Result<OptimizationResult, OptimizationError> {
        // 提升处理器优先级逻辑将由具体实现提供
...
impl UnifiedOptimizationManager {
    ...
    pub fn check_optimization(&mut self, syscall_num: u32) -> Option<OptimizationResult> {
        ...
        for i in 0..self.strategies.len() {
            let should_apply = { self.strategies[i].should_apply(&stats) };
            if should_apply {
                let result_opt = {
                    ...
                    match self.strategies[i].apply(&mut context) {
                        Ok(res) => Some(res),
                        Err(_) => None,
                    }
                };
                if let Some(result) = result_opt {
                    let strategy_name_owned = {
                        alloc::string::String::from(self.strategies[i].name())
                    };
                    self.record_optimization(syscall_num, strategy_name_owned.as_str(), &result, &stats);
                    return Some(result);
                }
            }
        }
