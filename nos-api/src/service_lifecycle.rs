...
use crate::{
    error::Result,
    interfaces::{
        InterfaceService as Service, InterfaceServiceInfo as ServiceInfo,
        InterfaceServiceManager as ServiceManager, InterfaceServiceStats as ServiceStats,
        InterfaceServiceStatus as ServiceStatus,
    },
};
...
impl ServiceManager for ServiceLifecycleManager {
    ...
    fn get_stats(&self) -> ServiceStats {
        ...
        let running_services = service_info
            .values()
...
pub enum ServiceLifecycleEvent {
    /// 服务注册
    ServiceRegistered { service_id: String, service_name: String },
    /// 服务启动
    ServiceStarted { service_id: String, service_name: String },
    /// 服务停止
    ServiceStopped { service_id: String, service_name: String },
    /// 服务错误
    ServiceError { service_id: String, service_name: String, error: String },
    /// 服务注销
        ...
        // 检查服务当前状态
        {
            ...
            if let Some(info) = info {
                match info.status {
                    ...
                    _ => {
                        return Err(crate::error::service_error(
                            "Service is not in a startable state",
                        ));
                    },
                }
            } else {
            ...
            if info_exists {
                ...
                if let Some(status) = status {
                    match status {
                        ...
                        _ => {
                            return Err(crate::error::service_error(
                                "Service is not in a stoppable state",
                            ));
                        },
                    }
                }
...
/// 全局服务生命周期管理器
static GLOBAL_SERVICE_MANAGER: Mutex<Option<Arc<Mutex<LifecycleAwareServiceManager>>>> =
    Mutex::new(None);
