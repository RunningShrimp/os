...
//!
//! 1. **Separation of Concerns**: Kernel code, data, heap, and physical mapping regions are kept
//!    separate to avoid address conflicts.
//! 2. **Architecture Compatibility**: Each architecture follows its standard memory layout
//!    conventions (e.g., Linux-style layouts).
//! 3. **ASLR Support**: Layout definitions support address space layout randomization for security.
//! 4. **Physical Mapping**: Direct physical memory mapping regions are separate from kernel code
//!    regions to prevent conflicts.
...
impl AddressSpaceLayout {
    ...
    pub fn is_kernel_code(&self, addr: usize) -> bool {
        addr >= self.kernel_code_base && addr < self.kernel_code_base + self.kernel_code_size
    }

    /// Check if an address is in kernel data region
    #[inline]
    pub fn is_kernel_data(&self, addr: usize) -> bool {
        addr >= self.kernel_data_base && addr < self.kernel_data_base + self.kernel_data_size
    }

    /// Check if an address is in kernel heap region
    #[inline]
    pub fn is_kernel_heap(&self, addr: usize) -> bool {
        addr >= self.kernel_heap_base && addr < self.kernel_heap_base + self.kernel_heap_size
    }
...
impl AddressSpaceLayout {
    ...
    pub fn apply_aslr_offset(&self, base: usize, offset: usize) -> usize {
        ...
        apply_aslr_offset_enhanced(base, offset, AslrRegionType::KernelCode).unwrap_or_else(|_| {
