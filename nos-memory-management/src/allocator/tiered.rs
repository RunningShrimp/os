...
impl MemoryPool {
        ...
        for (i, &block_header) in free_blocks.iter().enumerate() {
            ...
            if block.is_free() && block.size >= aligned_size {
                ...
                if block.size > aligned_size + core::mem::size_of::<BlockHeader>() + MIN_ALLOC_SIZE
                {
                    ...
                    let current_used =
                        self.used_bytes.fetch_add(aligned_size, Ordering::SeqCst) + aligned_size;
    ...
    pub unsafe fn deallocate(&self, ptr: *mut u8, _size: usize) {
        ...
        if !block.is_free() {
            ...
            self.used_bytes
                .fetch_sub(block.aligned_size(), Ordering::SeqCst);
    ...
    pub fn defragment(&self) {
        ...
        while i < free_blocks.len().saturating_sub(1) {
            ...
            if current_end == next_start {
                ...
                // Update fragmentation statistics
                if current_block.size < SMALL_OBJECT_THRESHOLD
                    || next_block.size < SMALL_OBJECT_THRESHOLD
                {
                    self.fragmentation_count.fetch_add(1, Ordering::SeqCst);
                }
            } else {
...
impl Clone for AllocatorStats {
    fn clone(&self) -> Self {
        Self {
            ...
            current_allocated_bytes: AtomicU64::new(
...
impl TieredMemoryAllocator {
    ...
    pub unsafe fn deallocate(&self, ptr: *mut u8, layout: Layout) {
        ...
        if size <= SMALL_OBJECT_THRESHOLD
            && let Some(pool) = &self.small_pool
        {
            unsafe { pool.deallocate(ptr, 0) };
        } else if size <= MEDIUM_OBJECT_THRESHOLD
            && let Some(pool) = &self.medium_pool
        {
            unsafe { pool.deallocate(ptr, 0) };
        } else if size <= LARGE_OBJECT_THRESHOLD
            && let Some(pool) = &self.large_pool
        {
            unsafe { pool.deallocate(ptr, 0) };
        }
    }
    ...
    pub fn get_memory_usage(&self) -> MemoryUsage {
        ...
        MemoryUsage {
            ...
            usage_ratio: if total_size > 0 {
                total_used as f64 / total_size as f64
            } else {
                0.0
            },
            fragmentation_ratio: (small_stats.fragmentation_ratio()
                + medium_stats.fragmentation_ratio()
                + large_stats.fragmentation_ratio())
                / 3.0,
