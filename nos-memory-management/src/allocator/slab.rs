        ...
        if let Some(ref slabs_array) = self.slabs {
            for (size_class_idx, slabs) in slabs_array.iter().enumerate() {
                ...
                for slab in slabs {
                    ...
                    debug_assert!(
                        slab.used_count <= slab.total_objects,
                        "Used count exceeds total objects"
                    );
                    debug_assert!(
                        slab.object_size == mem::size_of::<SlabObjectHeader>() + base_size,
...
mod tests {
    ...
    fn test_slab_alloc() {
        ...
        unsafe {
            alloc.init(0x1000 as *mut u8, 0x100000);
        }
