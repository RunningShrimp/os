...
pub fn gicv2_bases() -> Option<(usize, usize)> {
    *GICV2_BASES.lock()
}
static GICV3_BASES: Mutex<Option<(usize, usize)>> = Mutex::new(None);
pub fn gicv3_bases() -> Option<(usize, usize)> {
    *GICV3_BASES.lock()
}
static GICR_REDISTS: Mutex<Vec<(u64, usize)>> = Mutex::new(Vec::new());
pub fn gicr_redists() -> Vec<(u64, usize)> {
    GICR_REDISTS.lock().clone()
}
pub fn gicr_lookup(mpidr: u64) -> Option<usize> {
    GICR_REDISTS
        .lock()
        .iter()
        .find(|(m, _)| *m == mpidr)
...
pub fn gicr_default() -> Option<usize> {
    GICR_REDISTS.lock().get(0).map(|(_, b)| *b)
}
                ...
                best = match best {
                    ...
                    Some(b) => {
                        if r.weight > b.weight {
                            Some(r)
                        } else if r.weight < b.weight {
                            Some(b)
                        } else if (r.size as usize) > (b.size as usize) {
                            Some(r)
                        } else if (r.size as usize) < (b.size as usize) {
                            Some(b)
                        } else if r.order < b.order {
                            Some(r)
                        } else {
                            Some(b)
                        }
...
pub fn probe_dtb() {
    ...
    unsafe {
            ...
            match token {
                FDT_BEGIN_NODE => {
                    ...
                    if depth < cells_stack.len() {
                        cells_stack[depth] = current_cells;
                        ranges_stack[depth] = ranges.len();
                        weights_stack[depth] = current_weights.len();
                    }
                    depth += 1;
                ...
                FDT_END_NODE => {
                    ...
                    if depth > 0 {
                        depth -= 1;
                        current_cells = cells_stack[depth.min(cells_stack.len() - 1)];
                    }
                    ...
                    if current_weights.len() > wkeep {
                        current_weights.truncate(wkeep);
                    }
                    node_is_strong = false;
                    node_is_wc = false;
                    if node_is_gicr {
                ...
                FDT_PROP => {
                    ...
                    let prop_name = str::from_utf8(slice::from_raw_parts(prop_name_ptr, prop_len))
                        .unwrap_or("");

                    let data_ptr = p as *const u8;

                    if prop_name == "#address-cells" && len >= 4 {
                        current_cells.addr = u32::from_be(*(data_ptr as *const u32));
                    }
                    if prop_name == "#size-cells" && len >= 4 {
                        current_cells.size = u32::from_be(*(data_ptr as *const u32));
                    }
                    ...
                    if depth == 1 && prop_name == "compatible" {
                        ...
                        while off2 < len {
                            ...
                            let s2 =
                                str::from_utf8(slice::from_raw_parts(s_ptr2, sl2)).unwrap_or("");
                    ...
                    if depth == 1 && prop_name == "nos,mmio-decay-num" {
                        if len >= 4 {
                            cfg_decay_num = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if depth == 1 && prop_name == "nos,mmio-decay-den" {
                        if len >= 4 {
                            cfg_decay_den = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if depth == 1 && prop_name == "nos,mmio-threshold" {
                        if len >= 4 {
                            cfg_threshold_hits = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if depth == 1 && prop_name == "nos,mmio-cooldown-ticks" {
                        if len >= 4 {
                            cfg_cooldown_ticks = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if depth == 1 && prop_name == "nos,mmio-decay-interval" {
                        if len >= 4 {
                            cfg_decay_interval = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if depth == 1 && prop_name == "nos,mmio-sample-div" {
                        if len >= 4 {
                            cfg_sample_div = u32::from_be(*(data_ptr as *const u32)) as u64;
                        }
                    }
                    if prop_name == "nos,strong-compat" {
                        ...
                        while off3 < len {
                            ...
                            let s3 =
                                str::from_utf8(slice::from_raw_parts(s_ptr3, sl3)).unwrap_or("");
                            strong_list.push(s3);
                            off3 += sl3 + 1;
                        }
                    }
                    if prop_name == "nos,strong-device" {
                        node_is_strong = true;
                    }
                    if prop_name == "compatible" {
                        ...
                        while off < len {
                            ...
                            if s == "arm,gic-400" || s == "arm,gic-v2" {
                                node_is_gicv2 = true;
                            }
                            if s == "arm,gic-v3" {
                                node_is_gicv3 = true;
                            }
                            if s.contains("redistributor") || s.contains("gicr") {
                                node_is_gicr = true;
                            }
                            if s == "simple-framebuffer"
                    ...
                    if prop_name == "arm,mpidr" && len >= 8 {
                        current_gicr_mpidr = u64::from_be(*(data_ptr as *const u64));
                    }
                    ...
                    if prop_name == "ranges" {
                        ...
                        let p_ac = cells_stack[depth.saturating_sub(1).min(cells_stack.len() - 1)]
                            .addr as usize;
                        ...
                        while entry_cells > 0 && off + entry_cells * cell_bytes <= len {
                            let mut child: u128 = 0;
                            let mut parent: u128 = 0;
                            let mut size128: u128 = 0;
                            for i in 0..c_ac {
                                let v = u32::from_be(
                                    *((data_ptr.add(off + i * cell_bytes)) as *const u32),
                                ) as u128;
                                child = (child << 32) | v;
                            }
                            for i in 0..p_ac {
                                let v = u32::from_be(
                                    *((data_ptr.add(off + c_ac * cell_bytes + i * cell_bytes))
                                        as *const u32),
                                ) as u128;
                                parent = (parent << 32) | v;
                            }
                            for i in 0..sc {
                                let v = u32::from_be(
                                    *((data_ptr
                                        .add(off + (c_ac + p_ac) * cell_bytes + i * cell_bytes))
                                        as *const u32),
                                ) as u128;
                                size128 = (size128 << 32) | v;
                            }
                            let w = if idxw < current_weights.len() {
                                current_weights[idxw]
                            } else {
                                0
                            };
                            ranges.push(Range {
                        ...
                        if !current_weights.is_empty() && entries != current_weights.len() {
                            crate::println!(
                                "[dtb] ranges weight mismatch: entries={} weights={}",
                                entries,
                                current_weights.len()
                            );
                        }
                    ...
                    if prop_name == "reg" {
                        ...
                        if entry_cells > 0 {
                            ...
                            while off + entry_cells * cell_bytes <= len {
                                ...
                                for i in 0..sc {
                                    let v = u32::from_be(
                                        *((data_ptr.add(off + ac * cell_bytes + i * cell_bytes))
                                            as *const u32),
                                    ) as u128;
                                    size64 = (size64 << 32) | v;
                                }
                                ...
                                if node_name.starts_with("memory") {
                                    ...
                                    if size != 0 {
                                        if node_is_wc {
                                            mm::add_mmio_region_wc(addr, size);
                                        } else if node_is_strong {
                                            mm::add_mmio_region_strong(addr, size);
                                        } else {
                                            mm::add_mmio_region(addr, size);
                                        }
                                    }
                                }
                                ...
                                if node_is_gicr && size != 0 {
                                    current_gicr_addr = Some(addr);
                                }
                                off += entry_cells * cell_bytes;
                            }
                        }
        ...
        if max_mem_end != 0 {
            mm::set_phys_end(max_mem_end);
        }
        if cfg_decay_num != 0
            || cfg_decay_den != 0
            || cfg_threshold_hits != 0
            || cfg_cooldown_ticks != 0
            || cfg_decay_interval != 0
            || cfg_sample_div != 0
        {
            mm::mmio_cfg_update(
                if cfg_decay_interval != 0 {
                    Some(cfg_decay_interval)
                } else {
                    None
                },
                if cfg_decay_num != 0 {
                ...
                if cfg_threshold_hits != 0 {
                    Some(cfg_threshold_hits)
                } else {
                    None
                },
                if cfg_cooldown_ticks != 0 {
                    Some(cfg_cooldown_ticks)
                } else {
                    None
                },
                if cfg_sample_div != 0 {
