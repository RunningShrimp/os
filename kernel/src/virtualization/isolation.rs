...
//!
//! Namespace and Cgroup Isolation for the NOS kernel virtualization subsystem.
...
pub enum NamespaceType {
    Pid, Net, Ipc, Uts, Mount, Cgroup, User,
}
...
impl Namespace {
    pub fn create(ns_type: NamespaceType) -> Result<Self, ContainerError> {
        Ok(Self { ns_type, fd: None })
    }
    
    pub fn set_namespace(&self, _pid: u32) -> Result<(), ContainerError> { Ok(()) }
}

impl Cgroup {
    pub fn create(path: String, controllers: Vec<CgroupController>) -> Result<Self, ContainerError> {
        Ok(Self { path, controllers })
    }
    
    pub fn add_process(&self, _pid: u32) -> Result<(), ContainerError> { Ok(()) }
    pub fn set_memory_limit(&self, _limit: u64) -> Result<(), ContainerError> { Ok(()) }
    pub fn set_cpu_limit(&self, _quota: u64, _period: u64) -> Result<(), ContainerError> { Ok(()) }
}
...
impl SeccompFilter {
    ...
    pub fn add_rule(&mut self, rule: SeccompRule) { self.rules.push(rule); }
    pub fn apply(&self) -> Result<(), ContainerError> { Ok(()) }
}
