...
mod riscv64 {
    ...
    pub fn usertrap() {
        ...
        if scause == cause::USER_ECALL {
            ...
            // Exception
            println!("usertrap: unexpected scause={:#x} sepc={:#x} stval={:#x}",
                scause, sepc, stval);
        }
    }
    ...
    fn handle_interrupt(scause: usize) {
        match scause {
            ...
            _ => {
                println!("unexpected interrupt: {:#x}", scause);
            }
        }
    }
}
    ...
    pub fn handle_sync_el0() {
        ...
        match ec {
            ...
            ec::DATA_ABORT_LOWER | ec::INST_ABORT_LOWER => {
                println!("Page fault at {:#x}, esr={:#x}", far, esr);
            }
            _ => {
                println!("Unexpected exception: ec={:#x} esr={:#x} elr={:#x}",
                    ec, esr, elr);
            }
        }
    }
    ...
    pub fn trap_handler(vector: u8, error_code: usize, rip: usize) {
        match vector {
            ...
            vector::PAGE_FAULT => {
                ...
                println!("Page fault at {:#x}, error={:#x}, rip={:#x}",
                    cr2, error_code, rip);
            }
            vector::TIMER => {
                crate::time::timer_interrupt();
            ...
            _ => {
                println!("Unhandled trap: vector={} error={:#x} rip={:#x}",
