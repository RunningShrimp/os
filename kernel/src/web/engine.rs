...
use crate::graphics::compositor::{composite_frame, get_compositor};
...
impl WebEngineManager {
    ...
    pub fn new() -> Self {
        Self { engine: None, engine_type: None }
    }

    /// Initialize web engine
    pub fn init_engine(
        &mut self,
        engine_type: WebEngineType,
        width: u32,
        height: u32,
    ) -> Result<(), i32> {
...
static WEB_ENGINE_MANAGER: crate::subsystems::sync::Mutex<Option<WebEngineManager>> =
...
pub fn get_web_engine_manager() -> &'static crate::subsystems::sync::Mutex<WebEngineManager> {
    ...
    unsafe {
        &*(WEB_ENGINE_MANAGER.lock().as_ref().unwrap() as *const WebEngineManager
            as *const crate::subsystems::sync::Mutex<WebEngineManager>)
    }
}
