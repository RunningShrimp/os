...
pub struct RawSpinLock {
    locked: AtomicBool,
    /// CPU that holds the lock (-1 if unlocked)
...
impl RawSpinLock {
    pub const fn new() -> Self {
        Self {
            ...
            holder_cpu: AtomicUsize::new(usize::MAX),
        }
    }

    /// Acquire the lock with ticket-based fairness
    #[inline]
    pub fn lock(&self) {
        // Memory barrier before acquiring
        ...
        while self
            ...
            .is_err()
        {
            // Spin with hint for better performance on SMT processors
        ...
        self.holder_cpu.store(crate::cpu::cpuid(), Ordering::Relaxed);
        
        // Full memory barrier after acquiring
        fence(Ordering::SeqCst);
    }
    ...
    pub fn unlock(&self) {
        ...
        self.holder_cpu.store(usize::MAX, Ordering::Relaxed);
    ...
    /// Check if current CPU holds this lock
    #[inline]
    pub fn holding(&self) -> bool {
        self.is_locked() && self.holder_cpu.load(Ordering::Relaxed) == crate::cpu::cpuid()
    }
}
