...
impl Vfs {
    ...
    pub fn rename(&self, old_path: &str, new_path: &str) -> VfsResult<()> {
        ...
        let old_parent_inode = old_parent.lock().inode.clone();

        let (new_parent_path, new_name) = self.split_path(new_path)?;
        let new_parent = self.lookup_path(&new_parent_path)?;
        let new_parent_inode = new_parent.lock().inode.clone();

        old_parent_inode.rename(&old_name, &*new_parent_inode, &new_name)?;

        {
            let mut p_old = old_parent.lock();
            p_old.children.remove(&old_name);
        }
        {
            let mut p_new = new_parent.lock();
            let child_inode = old_parent_inode.lookup(&new_name)?;
            let child = Arc::new(Mutex::new(Dentry::new(new_name.clone(), child_inode, Some(new_parent.clone()))));
            p_new.children.insert(new_name.clone(), child.clone());
            let full = format!("{}/{}", new_parent_path, new_name);
            self.dentry_cache.lock().insert(full, child);
        }
        Ok(())
    }
    ...
    pub fn symlink(&self, target: &str, link_path: &str) -> VfsResult<()> {
        ...
        let parent_inode = parent.lock().inode.clone();
        let _inode = parent_inode.symlink(&name, target)?;
        let child_dentry = {
            let inode = parent_inode.lookup(&name)?;
            Arc::new(Mutex::new(Dentry::new(name.clone(), inode, Some(parent.clone()))))
        };
        {
            let mut p = parent.lock();
            p.children.insert(name.clone(), child_dentry.clone());
        }
        let full = format!("{}/{}", parent_path, name);
        self.dentry_cache.lock().insert(full, child_dentry);
        Ok(())
    }
...
impl VfsFile {
    ...
    pub fn read_at(&self, offset: u64, addr: usize, len: usize) -> Result<usize, ()> {
        let buf = unsafe { core::slice::from_raw_parts_mut(addr as *mut u8, len) };
        match self.inode.read(offset, buf) { Ok(n) => Ok(n), Err(_) => Err(()) }
    }

    pub fn write_at(&self, offset: u64, addr: usize, len: usize) -> Result<usize, ()> {
        let buf = unsafe { core::slice::from_raw_parts(addr as *const u8, len) };
        match self.inode.write(offset, &buf) { Ok(n) => Ok(n), Err(_) => Err(()) }
    }
...
pub mod ramfs {
    ...
    impl RamFsInode {
        ...
        fn rename(&self, old_name: &str, new_dir: &dyn InodeOps, new_name: &str) -> VfsResult<()> {
            ...
            if let Some(dir) = new_dir.as_any().downcast_ref::<RamFsInode>() {
                let mut nd = dir.children.lock();
                if nd.contains_key(new_name) { return Err(VfsError::Exists); }
                nd.insert(new_name.to_string(), inode);
                Ok(())
            } else {
                Err(VfsError::NotSupported)
            }
        ...
        fn symlink(&self, name: &str, target: &str) -> VfsResult<Arc<dyn InodeOps>> {
            let mut children = self.children.lock();
            if children.contains_key(name) { return Err(VfsError::Exists); }
            static NEXT_INO: AtomicUsize = AtomicUsize::new(1000);
            let ino = NEXT_INO.fetch_add(1, Ordering::Relaxed) as u64;
        ...
        fn readlink(&self) -> VfsResult<String> {
            ...
            if attr.mode.file_type() != FileType::Symlink { return Err(VfsError::InvalidOperation); }
            drop(attr);
            let data = self.data.lock();
            let s = core::str::from_utf8(&data).map_err(|_| VfsError::InvalidOperation)?;
            Ok(s.to_string())
        }
