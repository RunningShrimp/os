...
impl OptimizedBuddyAllocator {
    ...
    pub unsafe fn init(&mut self, heap_start: usize, heap_end: usize, min_block_size: usize) {
        ...
        // Initialize bitmap
        let bitmap_size = (self.total_blocks + 63) / 64;
        ...
        for i in order..32 {
            if !self.free_lists[i].is_null() {
                ...
                while current_order > order {
                    ...
                    let buddy_addr = unsafe { (current_block as usize) + block_size };
                ...
                let block_idx = unsafe { (current_block as usize - self.heap_start) / self.min_block_size };
                ...
                let aligned_ptr = align_up(unsafe { current_block as usize }, align) as *mut u8;
    ...
    pub fn dealloc(&mut self, ptr: *mut u8, layout: Layout) {
        ...
        let block_idx = unsafe { (block as usize - self.heap_start) / self.min_block_size };
    ...
    fn coalesce(&mut self, mut order: usize) {
        while order < self.max_order {
            ...
            let buddy_idx = unsafe { (buddy as usize - self.heap_start) / self.min_block_size };
