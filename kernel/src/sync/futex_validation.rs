...
impl FutexValidator {
    ...
    pub fn new() -> Self {
        Self { results: ValidationResult::default() }
    }
    ...
    fn validate_requeue_operations(&mut self) -> Result<(), &'static str> {
        ...
        if result.is_ok() {
            ...
            self.results.performance_metrics.avg_requeue_latency_ns =
                (self.results.performance_metrics.avg_requeue_latency_ns + (end_time - start_time))
                    / 2;
        } else {
            self.results.failed_tests += 1;
        }
    ...
    fn validate_timeout_operations(&mut self) -> Result<(), &'static str> {
        ...
        if result.is_err() {
            ...
            self.results.performance_metrics.avg_wait_latency_ns =
                (self.results.performance_metrics.avg_wait_latency_ns + (end_time - start_time))
                    / 2;
        } else {
            self.results.failed_tests += 1;
        }
    ...
    fn validate_performance(&mut self) -> Result<(), &'static str> {
        ...
        crate::println!(
            "[futex_validation] Avg wake latency: {}ns, Ops/sec: {:.2}",
            avg_time,
            self.results.performance_metrics.ops_per_second
        );
        Ok(())
    ...
    fn validate_memory_usage(&mut self) -> Result<(), &'static str> {
        ...
        self.results.total_tests += 1;
        if estimated_memory < 100 * 1024 {
            // Less than 100KB for 100 waiters
            self.results.passed_tests += 1;
        } else {
            self.results.failed_tests += 1;
        }
        ...
        crate::println!(
            "[futex_validation] Estimated memory usage: {} bytes for 100 waiters",
            estimated_memory
        );
        Ok(())
    }
    ...
    fn print_validation_summary(&self) {
        ...
        crate::println!(
            "[futex_validation] Avg wait latency: {}ns",
            self.results.performance_metrics.avg_wait_latency_ns
        );
        crate::println!(
            "[futex_validation] Avg wake latency: {}ns",
            self.results.performance_metrics.avg_wake_latency_ns
        );
        crate::println!(
            "[futex_validation] Avg requeue latency: {}ns",
            self.results.performance_metrics.avg_requeue_latency_ns
        );
        crate::println!(
            "[futex_validation] Operations per second: {:.2}",
            self.results.performance_metrics.ops_per_second
        );
        crate::println!(
            "[futex_validation] Memory usage: {} bytes",
...
pub fn run_futex_performance_benchmark() -> Result<(), &'static str> {
    ...
    for (name, iterations) in benchmarks {
        ...
        crate::println!(
            "[futex_benchmark] {}: {} iterations, total: {}ns, avg: {}ns, ops/sec: {:.2}",
            name,
            iterations,
            total_time,
            avg_time,
