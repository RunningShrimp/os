...
use crate::{
    graphics::{
        surface::{DirtyRect, Surface, SurfaceId, SurfaceManager, get_surface_manager},
        vsync::VsyncManager,
    },
...
impl Compositor {
    ...
    pub fn start(&self) -> Result<(), i32> {
        ...
        self.vsync.enable()?;
        crate::println!(
            "[compositor] Started compositor ({}x{})",
    ...
    pub fn composite(&self) -> Result<(), i32> {
        ...
        self.last_frame_time
            .store(frame_time as u32, Ordering::Relaxed);
    ...
    fn composite_surface(&self, surface: &Surface, surface_addr: usize) -> Result<(), i32> {
        ...
        let width =
            (self.framebuffer_width as i32 - dst_x).min(surface.width as i32 - src_x) as u32;
        let height =
            (self.framebuffer_height as i32 - dst_y).min(surface.height as i32 - src_y) as u32;
        ...
        if let Some(buffer_handle) = surface.front_buffer.as_ref().map(|b| b.handle) {
            ...
            if let Some(buf) = buffer {
                if buf.gpu_resource_id != 0 {
                    ...
                    return self.composite_surface_gpu(
                        &buf,
                        src_x as u32,
                        src_y as u32,
                        dst_x as u32,
        ...
        let src_stride = surface
            .front_buffer
            .as_ref()
            .map(|b| b.stride)
            .unwrap_or(surface.width * 4);
        let dst_stride = self.framebuffer_stride;

        unsafe {
            let src_ptr = (surface_addr
                + (src_y as usize * src_stride as usize)
                + (src_x as usize * 4)) as *const u8;
            let dst_ptr = (self.framebuffer_addr
                + (dst_y as usize * dst_stride as usize)
                + (dst_x as usize * 4)) as *mut u8;
    ...
    fn composite_surface_gpu(
        ...
        crate::println!(
            "[compositor] GPU-accelerated composition (resource: {}, {}x{})",
            buffer.gpu_resource_id,
            width,
            height
        );
        Ok(())
...
pub fn get_compositor() -> Option<&'static Compositor> {
    ...
    unsafe { compositor.as_ref().map(|c| &*(c as *const Compositor)) }
}
