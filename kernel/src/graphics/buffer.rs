...
impl BufferManager {
    ...
    pub fn allocate_buffer(
        ...
        if flags.scanout {
            if let Some(gpu_device) = crate::drivers::virtio_gpu::get_virtio_gpu() {
                return self.allocate_gpu_buffer(
                    handle,
                    width,
                    height,
                    format,
        ...
        crate::println!(
            "[graphics] Allocated buffer {} ({}x{}, type: {:?})",
            handle,
            width,
            height,
            buffer_type
    ...
    fn allocate_gpu_buffer(
        ...
        crate::println!(
            "[graphics] Allocated GPU-accelerated buffer {} ({}x{}, GPU resource: {})",
            handle,
            width,
            height,
            resource_id
    ...
    pub fn release_buffer(&self, handle: BufferHandle) -> Result<(), i32> {
        ...
        if buffer.gpu_resource_id != 0 {
            if let Some(gpu_device) = crate::drivers::virtio_gpu::get_virtio_gpu() {
                ...
                crate::println!(
                    "[graphics] Releasing GPU resource {} for buffer {}",
