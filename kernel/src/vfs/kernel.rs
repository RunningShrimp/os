...
pub fn create_root() -> VfsResult<Arc<dyn InodeOps>> {
    ...
    children.insert(
        "hostname".to_string(),
        Arc::new(SysFsInode::new_file(2002, Box::new(|| "nos\n".to_string()))),
    );

    // /sys/kernel/domainname - system domain name
    children.insert(
        "domainname".to_string(),
        Arc::new(SysFsInode::new_file(2003, Box::new(|| "(none)\n".to_string()))),
    );
    ...
    children.insert(
        "ostype".to_string(),
        Arc::new(SysFsInode::new_file(2005, Box::new(|| "NOS\n".to_string()))),
    );

    // /sys/kernel/metrics - runtime metrics
    children.insert(
        "metrics".to_string(),
        Arc::new(SysFsInode::new_file(2006, Box::new(|| format_kernel_metrics()))),
    );
