...
impl HealthIntegrationManager {
    ...
    fn process_health_result(&self, result: &HealthCheckResult) -> Result<(), i32> {
        match result.status {
            HealthStatus::Unhealthy => {
                crate::println!(
                    "[health-integration] Critical health issue detected: {} - {}",
                    result.name,
                    result.message
                );
            ...
            HealthStatus::Degraded => {
                crate::println!(
                    "[health-integration] Degraded health detected: {} - {}",
                    result.name,
                    result.message
                );
    ...
    fn trigger_critical_degradation(&self) -> Result<(), i32> {
        ...
        match manager.trigger_degradation(
            ...
            Ok(session_id) => {
                crate::println!(
                    "[health-integration] Triggered critical degradation: {}",
                    session_id
                );
                Ok(())
            },
            Err(e) => {
                ...
                crate::println!(
                    "[health-integration] Failed to trigger critical degradation (strategy may \
                     not exist): {}",
                    e
                );
    ...
    fn trigger_degraded_degradation(&self) -> Result<(), i32> {
        ...
        match manager.trigger_degradation(
            ...
            Ok(session_id) => {
                crate::println!(
                    "[health-integration] Triggered degraded degradation: {}",
                    session_id
                );
                Ok(())
            },
            Err(e) => {
                // Strategy might not exist, log and continue
                crate::println!(
                    "[health-integration] Failed to trigger degraded degradation (strategy may \
                     not exist): {}",
                    e
                );
                // Don't return error - this is best-effort
    ...
    fn trigger_component_degradation(&self, component: &str, severity: &str) -> Result<(), i32> {
        ...
        match manager.trigger_degradation(
            ...
            Err(_) => {
                // Strategy might not exist, log and continue
                crate::println!(
                    "[health-integration] Failed to trigger component degradation for {} \
...
pub fn trigger_degradation_from_error_handling(
    component: &str,
    severity: &str,
    message: &str,
) -> Result<(), i32> {
    if let Some(manager_mutex) = get_health_integration_manager() {
        ...
        crate::println!(
            "[health-integration] Warning: Health integration not initialized, cannot trigger \
             degradation"
        );
        Ok(())
    }
