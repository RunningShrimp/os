/// Host Intrusion Detection System (HIDS) - Type Definitions
///
/// This module contains all type definitions for the host-based intrusion detection system.
/// 包括数据结构、枚举类型和相关的类型定义。
...
use crate::ids::{Evidence, ThreatLevel};
...
pub struct SyscallAnomalyDetector {
    /// 异常模型
    pub models: BTreeMap<u32, SyscallAnomalyModel>,
    /// 阈值设置
    pub thresholds: SyscallThresholds,
}
...
pub struct ChangeDetector {
    /// 文件变化历史
    pub change_history: BTreeMap<String, Vec<FileChange>>,
    /// 检测模式
...
pub struct ProcessTree {
    ...
    /// 进程关系
    pub parent_child_map: BTreeMap<u64, Vec<u64>>,
    /// 进程信息
    pub process_info: BTreeMap<u64, ProcessInfo>,
}

/// 进程行为检测器
pub struct ProcessBehaviorDetector {
    /// 行为模型
    pub behavior_models: BTreeMap<String, ProcessBehaviorModel>,
    /// 异常阈值
    pub anomaly_thresholds: ProcessAnomalyThresholds,
}
...
pub struct PrivilegeMonitor {
    ...
    /// 监控规则
    pub monitor_rules: Vec<PrivilegeRule>,
}
...
pub struct PrivilegeModel {
    /// 用户权限
    pub user_privileges: BTreeMap<u32, UserPrivileges>,
    /// 组权限
    pub group_privileges: BTreeMap<u32, GroupPrivileges>,
    /// 能力列表
    pub capabilities: BTreeMap<u32, Vec<String>>,
}
...
pub struct PrivilegeUidCondition {
    /// 操作符
    pub operator: crate::ids::network_ids::ComparisonOperator,
    /// UID值
    pub uid: u32,
}
...
pub struct StartupMonitor {
    /// 监控的启动项
    pub monitored_items: Vec<StartupItem>,
    /// 启动项变化历史
...
pub struct NetworkAnomalyDetector {
    /// 异常模型
    pub models: Vec<NetworkAnomalyModel>,
    /// 检测阈值
...
pub struct UserBehaviorAnalyzer {
    /// 用户行为模型
    pub user_models: BTreeMap<u32, UserBehaviorModel>,
    /// 行为模式
...
pub struct HeuristicEngine {
    ...
    /// 评分系统
    pub scoring_system: HeuristicScoringSystem,
}
