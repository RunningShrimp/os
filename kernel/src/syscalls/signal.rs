...
pub fn dispatch(syscall_id: u32, args: &[u64]) -> SyscallResult {
    match syscall_id {
        ...
        0x500F => sys_signal(args),          // signal (BSD compatibility)
...
fn sys_sigaction(args: &[u64]) -> SyscallResult {
    use super::common::extract_args;
    use crate::{
        ipc::signal::{SigAction, SIG_DFL, SIG_IGN},
        mm::vm::{copyin, copyout},
    };
    ...
    let _proc_pid = proc.pid; // Use proc for validation
    ...
    if !oldact_ptr.is_null() {
        ...
        unsafe {
            copyout(
                pagetable,
                oldact_ptr as usize,
                core::ptr::addr_of!(old_action) as *const u8,
    ...
    if !act_ptr.is_null() {
        ...
        unsafe {
            copyin(
                pagetable,
                core::ptr::addr_of_mut!(new_action) as *mut u8,
                act_ptr as usize,
                core::mem::size_of::<SigAction>(),
            )
            .map_err(|_| SyscallError::BadAddress)?;
        ...
        if handler != SIG_DFL && handler != SIG_IGN && (handler as usize) < crate::mm::vm::USER_BASE
        {
            return Err(SyscallError::InvalidArgument);
        }

        // Set new action
        let old_action = signals
...
fn sys_sigprocmask(args: &[u64]) -> SyscallResult {
    use super::common::extract_args;
    use crate::{
        ipc::signal::{SigSet, SIG_BLOCK, SIG_SETMASK, SIG_UNBLOCK},
        mm::vm::{copyin, copyout},
    };
    ...
    if !oldset_ptr.is_null() {
        ...
        unsafe {
            copyout(
                pagetable,
                oldset_ptr as usize,
                core::ptr::addr_of!(old_mask) as *const u8,
    ...
    let new_set = if !set_ptr.is_null() {
        ...
        unsafe {
            copyin(
                pagetable,
                core::ptr::addr_of_mut!(set) as *mut u8,
                set_ptr as usize,
                core::mem::size_of::<SigSet>(),
            )
            .map_err(|_| SyscallError::BadAddress)?;
...
fn sys_sigpending(args: &[u64]) -> SyscallResult {
    use super::common::extract_args;
    use crate::{ipc::signal::SigSet, mm::vm::copyout};
    ...
    unsafe {
        copyout(
            pagetable,
            set_ptr as usize,
            core::ptr::addr_of!(pending) as *const u8,
            core::mem::size_of::<SigSet>(),
        )
        .map_err(|_| SyscallError::BadAddress)?;
    }
...
fn sys_sigsuspend(args: &[u64]) -> SyscallResult {
    use super::common::extract_args;
    use crate::{ipc::signal::SigSet, mm::vm::copyin};
    ...
    unsafe {
        copyin(
            pagetable,
            core::ptr::addr_of_mut!(new_mask) as *mut u8,
            mask_ptr as usize,
            core::mem::size_of::<SigSet>(),
        )
        .map_err(|_| SyscallError::BadAddress)?;
...
fn sys_sigaltstack(args: &[u64]) -> SyscallResult {
    ...
    if !old_ss_ptr.is_null() {
        ...
        unsafe {
            copyout(
                pagetable,
                old_ss_ptr as usize,
                core::ptr::addr_of!(old_stack) as *const u8,
    ...
    if !ss_ptr.is_null() {
        ...
        unsafe {
            copyin(
                pagetable,
                core::ptr::addr_of_mut!(new_stack) as *mut u8,
                ss_ptr as usize,
                core::mem::size_of::<StackT>(),
            )
            .map_err(|_| SyscallError::BadAddress)?;
...
fn sys_rt_sigtimedwait(args: &[u64]) -> SyscallResult {
    ...
    // Validate timeout_ptr (can be null for infinite wait)
    // Use timeout_ptr to determine if timeout is specified
    let has_timeout = !timeout_ptr.is_null();
    if has_timeout {
        // TODO: Read timeout from user space using timeout_ptr
        let _timeout_addr = timeout_ptr as usize; // Use timeout_ptr for
                                                  // validation
    }
    ...
    unsafe {
        copyin(
            pagetable,
            core::ptr::addr_of_mut!(wait_set) as *mut u8,
            set_ptr as usize,
            core::mem::size_of::<SigSet>(),
        )
        .map_err(|_| SyscallError::BadAddress)?;
        ...
        if !info_ptr.is_null() {
            unsafe {
                copyout(
                    pagetable,
                    info_ptr as usize,
                    core::ptr::addr_of!(info) as *const u8,
...
fn sys_rt_sigqueueinfo(args: &[u64]) -> SyscallResult {
    use super::common::extract_args;
    use crate::{ipc::signal::SigInfo, mm::vm::copyin};
            ...
            if !info_ptr.is_null() {
                ...
                if !pagetable.is_null() {
                    unsafe {
                        copyin(
                            pagetable,
                            core::ptr::addr_of_mut!(info) as *mut u8,
                            info_ptr as usize,
                            core::mem::size_of::<SigInfo>(),
                        )
                        .map_err(|_| SyscallError::BadAddress)?;
