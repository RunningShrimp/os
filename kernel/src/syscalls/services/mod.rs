...
// 重新导出常用的类型和特征，方便外部使用
pub use traits::{
    Service, ServiceFactory, ServiceHealth, ServiceLifecycle, ServiceProvider, ServiceStatus,
    SyscallService,
};
...
pub fn init_service_system() -> Result<
    (alloc::sync::Arc<ServiceRegistry>, alloc::sync::Arc<SyscallDispatcher>),
    crate::error_handling::unified::KernelError,
> {
    use alloc::sync::Arc;
...
impl ServiceSystem {
    ...
    pub fn handle_syscall(
        ...
        syscall_number: u32,
        args: &[u64],
    ) -> Result<u64, crate::error_handling::unified::KernelError> {
        ...
        if result.success {
            ...
            Err(result
                .error
                .unwrap_or(crate::error_handling::unified::KernelError::Unknown))
        }
    }
...
impl Clone for SystemStats {
    fn clone(&self) -> Self {
        Self {
            dispatch_stats: self.dispatch_stats.clone(),
