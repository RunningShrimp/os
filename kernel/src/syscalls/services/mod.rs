...
impl ServiceSystem {
    ...
    pub fn handle_syscall(&self, syscall_number: u32, args: &[u64]) -> Result<u64, crate::error_handling::unified::KernelError> {
        ...
        if result.success {
            ...
            Err(result.error.unwrap_or(crate::error_handling::unified::KernelError::Unknown))
        }
    }
    ...
    pub fn start_all_services(&self) -> Result<(), crate::error_handling::unified::KernelError> {
        ...
        for service_name in startup_order {
            ...
            // 使用 service_name 获取服务状态并启动
            if let Ok(status) = self.registry.get_service_status(&service_name) {
                // 如果服务未初始化，尝试启动它
                // TODO: 实现服务启动逻辑
                let _service_status = status; // 使用服务状态进行验证
            }
        }
