...
use alloc::{string::ToString, collections::BTreeMap, string::String, sync::Arc, vec::Vec};
...
impl SyscallDispatcher {
    ...
    fn find_service_for_syscall(&self, syscall_number: u32) -> Result<String, KernelError> {
        ...
        let service_name = self
            .registry
            .get_syscall_service(syscall_number)?
            .ok_or_else(|| KernelError::NotSupported)?;
    ...
    fn execute_syscall(
        ...
        while retries <= self.config.max_retries {
            match self.try_execute_syscall(service_name, syscall_number, args, start_time) {
                ...
                Err(KernelError::ResourceBusy) if retries < self.config.max_retries => {
                    retries += 1;
                    // 简单的退避策略
                    // println removed for no_std compatibility
                },
    ...
    fn check_cache(&self, syscall_number: u32) -> Option<CachedServiceInfo> {
        ...
        let cache = self.syscall_cache.lock();
