...
impl EnhancedMathLib {
    ...
    pub fn tan(&self, x: c_double) -> c_double {
        ...
        if cos_val.abs() < math_constants::DBL_EPSILON {
            set_errno(EDOM);
            return if cos_val > 0.0 {
                math_constants::DBL_MAX
            } else {
                -math_constants::DBL_MAX
            };
        }
    ...
    fn erf_approximation(&self, x: c_double) -> c_double {
        ...
        let y =
            1.0 - (((((A5 * tau + A4) * tau) + A3) * tau + A2) * tau + A1) * tau * (-x_abs * x_abs);
...
#[inline]
pub fn cbrt(x: c_double) -> c_double {
    MATH_LIB.cbrt(x)
}
#[inline]
...
#[inline]
pub fn trunc(x: c_double) -> c_double {
    MATH_LIB.trunc(x)
}
#[inline]
...
#[inline]
pub fn lgamma(x: c_double) -> c_double {
    MATH_LIB.lgamma(x)
}
