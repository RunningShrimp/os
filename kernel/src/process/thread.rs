...
pub fn schedule() {
    ...
    // Real-time scheduling: first check for RT threads
    let mut next_tid = find_realtime_thread(current_tid);
    ...
    if let Some(tid) = next_tid {
        ...
        if let Some(thread) = table.find_thread(tid) {
            ...
            // In real implementation, this would call architecture-specific context switch
...
pub fn thread_cancel(tid: Tid) -> Result<(), ThreadError> {
    ...
    if thread.cancel_state == CancelState::Disabled {
    ...
    if thread.cancel_type == CancelType::Asynchronous {
        thread_exit(null_mut());
    }
