...
impl Default for MqNotify {
    fn default() -> Self {
        Self { notify_method: MQ_SIGNAL, notify_sig: 0, notify_pid: 0 }
    }
}
...
impl Default for MqAttr {
    fn default() -> Self {
        Self { mq_maxmsg: 10, mq_msgsize: 8192, mq_curmsgs: 0, mq_flags: 0 }
    }
}
...
// Safety: MessageQueue uses proper synchronization primitives, so raw pointers to it can be
// Send/Sync
...
/// Global message queue registry
static MESSAGE_QUEUES: Mutex<BTreeMap<String, alloc::sync::Arc<MessageQueue>>> =
...
static MQD_TABLE: Mutex<BTreeMap<usize, alloc::sync::Arc<MessageQueue>>> =
    Mutex::new(BTreeMap::new());
...
pub extern "C" fn mq_send(
    ...
    unsafe {
        ...
        core::ptr::copy_nonoverlapping(msg_ptr as *const u8, msg_data.as_mut_ptr(), msg_len);
    }
    ...
    crate::println!(
        "[mqueue] Sent message to queue '{}', len={}, prio={}",
        mq.name,
        msg_len,
        msg_prio
    );
...
pub extern "C" fn mq_receive(
    ...
    crate::println!(
        "[mqueue] Received message from queue '{}', len={}, prio={}",
        mq.name,
        message.data.len(),
        message.priority
    );
...
pub extern "C" fn mq_getattr(mqd: i32, attr: *mut MqAttr) -> i32 {
    ...
    crate::println!(
        "[mqueue] Got attributes for queue '{}': maxmsg={}, msgsize={}, curmsgs={}",
        mq.name,
        mq_attr.mq_maxmsg,
        mq_attr.mq_msgsize,
        mq_attr.mq_curmsgs
...
pub extern "C" fn mq_setattr(mqd: i32, attr: *const MqAttr) -> i32 {
    ...
    crate::println!(
        "[mqueue] Set attributes for queue '{}': maxmsg={}, msgsize={}, flags={}",
        mq.name,
        mq_attr.mq_maxmsg,
        mq_attr.mq_msgsize,
        mq_attr.mq_flags
...
pub extern "C" fn mq_notify(mqd: i32, notification: *const MqNotify) -> i32 {
    ...
    *notify = Some(notify_info);
    ...
    crate::println!(
        "[mqueue] Registered notification for queue '{}': method={}, sig={}, pid={}",
        mq.name,
        notify_info.notify_method,
        notify_info.notify_sig,
        notify_info.notify_pid
