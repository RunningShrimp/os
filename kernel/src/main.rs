...
fn test_pipe_fork_rw() {
    ...
    let mut pfds = [0i32; 2];
    let ret = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Pipe as usize,
    ...
    let pid =
        crate::syscalls::dispatch(crate::syscalls::SysNum::Fork as usize, &[0, 0, 0, 0, 0, 0]);
    if pid == 0 {
        // child
        // Read from pipe
        let mut buf = [0u8; 8];
        let r = crate::syscalls::dispatch(
            crate::syscalls::SysNum::Read as usize,
            &[
                pfds[0] as usize,
        ...
        let _ = crate::syscalls::dispatch(
            crate::syscalls::SysNum::Close as usize,
            &[pfds[0] as usize, 0, 0, 0, 0, 0],
        );
        let _ =
            crate::syscalls::dispatch(crate::syscalls::SysNum::Exit as usize, &[0, 0, 0, 0, 0, 0]);
    } else {
        // parent
        let _ = crate::syscalls::dispatch(
            crate::syscalls::SysNum::Fcntl as usize,
            &[
                pfds[1] as usize,
                crate::posix::F_SETFL as usize,
                O_NONBLOCK as usize,
                0,
                0,
                0,
        ...
        let _ = crate::syscalls::dispatch(
            crate::syscalls::SysNum::Write as usize,
            &[pfds[1] as usize, b"hello".as_ptr() as usize, 5, 0, 0, 0],
        );
        let mut status = 0i32;
        let _ = crate::syscalls::dispatch(
            crate::syscalls::SysNum::Wait as usize,
...
fn test_exec_negative() {
    ...
    let ret = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Exec as usize,
...
fn test_exec_positive_minimal() {
    ...
    #[cfg(target_arch = "riscv64")]
    {
        elf[16 + 18] = (243u16 & 0xFF) as u8;
        elf[16 + 19] = (243u16 >> 8) as u8;
    }
    #[cfg(target_arch = "aarch64")]
    {
        elf[16 + 18] = (183u16 & 0xFF) as u8;
        elf[16 + 19] = (183u16 >> 8) as u8;
    }
    #[cfg(target_arch = "x86_64")]
    {
        elf[16 + 18] = (62u16 & 0xFF) as u8;
        elf[16 + 19] = (62u16 >> 8) as u8;
    }
    // e_version
    elf[16 + 20] = 1;
    elf[16 + 21] = 0;
    ...
    let path = "/bin/hello";
    let mut f = vfs()
        .create(path, FileMode::new(FileMode::S_IFREG | 0o755))
...
fn test_paths_relative() {
    ...
    // mkdir /tmp and chdir
    let _ = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Mkdir as usize,
        ["/tmp\0".as_ptr() as usize, 0, 0, 0, 0, 0].as_ref(),
    );
    let _ = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Chdir as usize,
        ["/tmp\0".as_ptr() as usize, 0, 0, 0, 0, 0].as_ref(),
    );
    ...
    let fd = crate::syscalls::dispatch(
        ...
        [
            "foo\0".as_ptr() as usize,
            (O_CREAT | O_RDWR) as usize,
            0o644,
            0,
            0,
            0,
    ...
    let _ = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Write as usize,
        [fd as usize, b"ok".as_ptr() as usize, 2, 0, 0, 0].as_ref(),
    );
    // link to bar
    let _ = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Link as usize,
        [
            "foo\0".as_ptr() as usize,
            "bar\0".as_ptr() as usize,
            0,
            0,
            0,
            0,
    ...
    // unlink foo
    let _ = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Unlink as usize,
        ["foo\0".as_ptr() as usize, 0, 0, 0, 0, 0].as_ref(),
    );
    // open absolute /tmp/bar
    let fd2 = crate::syscalls::dispatch(
        crate::syscalls::SysNum::Open as usize,
