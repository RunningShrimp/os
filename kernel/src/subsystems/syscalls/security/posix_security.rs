...
fn sys_capget(args: &[u64]) -> SyscallResult {
    ...
    let security_context = get_current_security_context();

    // Check enhanced permissions for capget operation
    let object = Object::SystemResource("capabilities");
    let requested_permissions = PermissionBits::SECURITY_AUDIT;

    match check_permission(&security_context, &object, requested_permissions) {
        AccessResult::Allow => {},
        AccessResult::Deny { .. } => {
...
fn sys_capset(args: &[u64]) -> SyscallResult {
    ...
    // Check enhanced permissions for capset operation
    let object = Object::SystemResource("capabilities");
    let requested_permissions = PermissionBits::SECURITY_CONFIGURE;
        ...
        Ok(entry) => {
            ...
            if pwd_ptr != 0 {
                let entry_data = unsafe { 
                    ...
                    let mut entry_bytes = [0u8; 512]; // Enough space for entry
                    let mut offset = 0;
        ...
        Ok(entry) => {
            ...
            if grp_ptr != 0 {
                let entry_data = unsafe { 
                    ...
                    // Create structure layout
                    let mut entry_bytes = [0u8; 1024]; // Enough space for entry
                    let mut offset = 0;
