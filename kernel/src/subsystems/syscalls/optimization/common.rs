...
use crate::subsystems::syscalls::optimization::core::{
    OptimizationConfig, SyscallStatsSnapshot, UnifiedSyscallStats,
};
...
impl FileDescriptorValidator {
    ...
    pub fn get_file_index(fd: i32) -> Result<usize, crate::syscalls::common::SyscallError> {
        ...
        let pid = crate::process::myproc()
            .ok_or(crate::syscalls::common::SyscallError::InvalidArgument)?;
...
impl PerformanceMonitor {
    ...
    pub fn record_syscall_performance(
        ...
        // 记录全局统计
        self.global_stats
            .record_call(duration_ns, success, cache_hit);

        // 记录特定系统调用统计
        let mut stats_map = self.syscall_stats.lock();
        let stats = stats_map
            .entry(syscall_num)
            .or_insert_with(UnifiedSyscallStats::new);
    ...
    pub fn get_all_syscall_stats(&self) -> BTreeMap<u32, SyscallStatsSnapshot> {
        ...
        stats_map
            .iter()
            .map(|(num, stats)| (*num, stats.get_snapshot()))
            .collect()
    }
