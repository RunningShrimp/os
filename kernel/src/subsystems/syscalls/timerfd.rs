...
use crate::subsystems::{sync::Once, time};
...
static mut TIMERFD_INSTANCES: Option<Vec<Option<TimerFdInstance>>> = None;
static TIMERFD_INSTANCES_INIT: Once = Once::new();

fn init_timerfd_instances() {
    unsafe {
        if TIMERFD_INSTANCES.is_none() {
            TIMERFD_INSTANCES = Some(Vec::new());
        }
    }
}
...
fn alloc_timerfd_instance(clock_id: i32, flags: i32) -> Option<usize> {
    unsafe {
        ...
        TIMERFD_INSTANCES_INIT.call_once(|| {
            init_timerfd_instances();
        });

        let instances = TIMERFD_INSTANCES.as_mut().unwrap();
...
impl SyscallHandler for TimerFdHandler {
    fn handle(&self, _args: &[u64]) -> SyscallResult<()> {
        ...
        Err(InterfaceSyscallError::NotSupported)
    }
...
pub fn sys_timerfd_create(args: &[u64]) -> SyscallResult<i64> {
    ...
    let instance_idx = alloc_timerfd_instance(clockid, flags).ok_or(InterfaceSyscallError::OutOfMemory)?;
    ...
    if let Some(proc) = proc_table.find(pid) {
        ...
        for (fd, file_idx_ref) in proc.ofile.iter_mut().enumerate() {
            if file_idx_ref.is_none() {
                ...
                if let Some(file) = file_table.get_mut(file_idx) {
                    // Set the file type and properties
                    file.ftype = crate::subsystems::fs::file::FileType::TimerFd;
                    file.readable = true;
                    file.writable = false;
...
pub fn sys_timerfd_settime(args: &[u64]) -> SyscallResult<i64> {
    ...
    let flags = args[1] as i32;
    let _new_value_ptr = args[2] as usize;
    let _old_value_ptr = args[3] as usize;
    ...
    if let Some(proc) = proc_table.find(pid) {
        if let Some(file_idx) = proc.ofile[fd as usize] {
            ...
            if let Some(file) = file_table.get(file_idx) {
                ...
                let instance_idx = file.timerfd_instance.ok_or(InterfaceSyscallError::InvalidArgument)?;
                drop(proc_table);

                if let Some(instance) = get_timerfd_instance(instance_idx) {
                    // For now, just use a dummy value since copyin/copyout are not implemented
                    let new_value = 0u64;
                    let new_interval = 0u64;

                    // Get old value before setting
                    let (_old_value, _old_interval) = instance.get_time();
...
pub fn sys_timerfd_gettime(args: &[u64]) -> SyscallResult<i64> {
    ...
    let fd = args[0] as i32;
    let _curr_value_ptr = args[1] as usize;
