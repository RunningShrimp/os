...
use crate::syscall_interface::{ServiceManager, ServiceStats, Service};
...
// 重新导出常用的类型和特征，方便外部使用
pub use traits::{
    Service as BaseService, SyscallService, ServiceLifecycle, ServiceFactory, ServiceProvider,
...
pub fn init_service_system() -> nos_api::Result<(alloc::sync::Arc<ServiceRegistry>, alloc::sync::Arc<SyscallDispatcher>)> {
    use alloc::sync::Arc;
...
impl ServiceSystem {
    ...
    pub fn handle_syscall(&self, syscall_number: u32, args: &[u64]) -> nos_api::Result<u64> {
        ...
        if result.success {
            ...
            Err(result.error.unwrap_or(nos_api::Error::SystemError("Unknown syscall error".into())))
        }
    }
...
impl ServiceManager for ServiceSystem {
    ...
    fn list_services(&self) -> Vec<&str> {
        ...
        services.into_iter().map(|s| Box::leak(s.into_boxed_str())).collect()
    }
