...
/// Get current process PID (public API)
pub fn current_pid() -> Pid {
    myproc().unwrap_or(0)
}

/// Get current thread ID (same as PID in this implementation)
pub fn current_thread_id() -> u64 {
    myproc().map(|pid| pid as u64).unwrap_or(0)
}
...
pub fn terminate_process(pid: u64) {
    ...
    if let Some(proc) = table.find(pid as Pid) {
        ...
        if myproc() == Some(pid as Pid) {
            drop(table); // Release lock before yielding
            yield_cpu();
        }
    }
}
