...
static CONTEXT_SWITCH_STATS_INIT: crate::subsystems::sync::Once =
    crate::subsystems::sync::Once::new();
...
#[cfg(target_arch = "x86_64")]
unsafe fn x86_64_context_switch(
    current: &mut Context,
    next: &Context,
) -> Result<(), ContextSwitchError> {
...
#[cfg(target_arch = "riscv64")]
unsafe fn riscv64_context_switch(
    current: &mut Context,
    next: &Context,
) -> Result<(), ContextSwitchError> {
    // Validate stack pointers
...
#[cfg(target_arch = "aarch64")]
unsafe fn aarch64_context_switch(
    current: &mut Context,
    next: &Context,
) -> Result<(), ContextSwitchError> {
    // Validate stack pointers
...
pub fn get_current_context() -> Option<&'static mut Context> {
    crate::subsystems::process::thread::get_current_thread_mut().map(|thread| &mut thread.context)
}
