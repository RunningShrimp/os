...
struct MutexState {
    ...
    /// Total times lock had to wait (contention)
    lock_contended_count: AtomicU64,
}
...
impl MutexState {
    const fn new() -> Self {
        Self {
            ...
            lock_contended_count: AtomicU64::new(0),
        }
    }
}
...
impl<T: Send + Sync> MutexEnhanced<T> {
    ...
    /// Get lock analytics snapshot: (acquire_count, contended_count)
    pub fn stats(&self) -> (u64, u64) {
        let acquires = self.state.lock_acquire_count.load(Ordering::Relaxed);
        let contended = self.state.lock_contended_count.load(Ordering::Relaxed);
        (acquires, contended)
    }
}
