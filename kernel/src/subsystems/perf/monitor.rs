...
impl PerformanceMonitor {
    ...
    pub fn get_metrics_summary(&self) -> BTreeMap<String, String> {
        ...
        summary
            .insert("cpu_usage_percent".to_string(), format!("{:.2}", self.calculate_cpu_usage()));
        summary.insert(
            "cpu_frequency_mhz".to_string(),
            format!("{}", snapshot.cpu_metrics.frequency_mhz),
        );
        summary.insert(
            "context_switches".to_string(),
            format!("{}", snapshot.cpu_metrics.context_switches),
        );
        summary.insert("interrupts".to_string(), format!("{}", snapshot.cpu_metrics.interrupts));

        summary.insert(
            "memory_usage_percent".to_string(),
            format!("{:.2}", self.calculate_memory_usage()),
        );
        summary.insert(
            "memory_used_bytes".to_string(),
            format!("{}", snapshot.memory_metrics.used_bytes),
        );
        summary.insert(
            "memory_free_bytes".to_string(),
            format!("{}", snapshot.memory_metrics.free_bytes),
        );
        summary
            .insert("page_faults".to_string(), format!("{}", snapshot.memory_metrics.page_faults));

        summary.insert("io_bytes_read".to_string(), format!("{}", snapshot.io_metrics.bytes_read));
        summary.insert(
            "io_bytes_written".to_string(),
            format!("{}", snapshot.io_metrics.bytes_written),
        );
        summary.insert(
            "io_active_requests".to_string(),
            format!("{}", snapshot.io_metrics.active_requests),
        );

        summary.insert(
            "network_bytes_received".to_string(),
            format!("{}", snapshot.network_metrics.bytes_received),
        );
        summary.insert(
            "network_bytes_sent".to_string(),
            format!("{}", snapshot.network_metrics.bytes_sent),
        );
        summary.insert(
            "network_active_connections".to_string(),
            format!("{}", snapshot.network_metrics.active_connections),
        );

        summary.insert(
            "scheduler_total_tasks".to_string(),
            format!("{}", snapshot.scheduler_metrics.total_tasks),
        );
        summary.insert(
            "scheduler_running_tasks".to_string(),
            format!("{}", snapshot.scheduler_metrics.running_tasks),
        );
        summary.insert(
            "scheduler_average_load".to_string(),
            format!("{:.2}", snapshot.scheduler_metrics.average_load),
        );
