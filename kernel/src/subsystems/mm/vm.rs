...
pub fn init() {
    ...
    *KERNEL_PAGETABLE.lock() = PageTablePtr(pt as *mut PageTable);
    ...
    let phys_map_base = if aslr_enabled {
        // Use enhanced ASLR function with region awareness and boundary checks
        use nos_memory_management::layout::{AddressSpaceLayout, AslrRegionType};
        AddressSpaceLayout::current()
            .apply_aslr_offset_enhanced(
                base_phys_map,
                aslr_offset,
                AslrRegionType::PhysicalMapping,
            )
            .unwrap_or_else(|e| {
                panic!("vm::init: Failed to apply ASLR offset to physical mapping: {}", e);
            })
    } else {
        base_phys_map
    };
    ...
    // Enhanced verification is now done inside apply_aslr_offset_enhanced
    // Additional verification for kernel regions
    if aslr_enabled {
        use nos_memory_management::layout::{AddressSpaceLayout, AslrRegionType};
        let layout = AddressSpaceLayout::current();
        
        // Apply ASLR to kernel code region (if needed) for verification
        let kernel_code_base = layout.apply_aslr_offset_enhanced(
            layout.kernel_code_base,
            aslr_offset,
            AslrRegionType::KernelCode,
        ).unwrap_or(layout.kernel_code_base);
        
        // Apply ASLR to kernel data region (if needed) for verification
        let kernel_data_base = layout.apply_aslr_offset_enhanced(
            layout.kernel_data_base,
            aslr_offset,
            AslrRegionType::KernelData,
        ).unwrap_or(layout.kernel_data_base);
        
        // Verify no overlaps
        if kernel_code_base + layout.kernel_code_size > kernel_data_base {
            panic!("vm::init: ASLR-adjusted kernel code overlaps with kernel data");
        }
        if kernel_data_base + layout.kernel_data_size > layout.kernel_heap_base {
            panic!("vm::init: ASLR-adjusted kernel data overlaps with kernel heap");
        }
        if phys_map_base < layout.kernel_base + layout.kernel_code_size + layout.kernel_data_size + layout.kernel_heap_size {
            panic!("vm::init: ASLR-adjusted phys_map_base overlaps with kernel regions");
        }
    }
