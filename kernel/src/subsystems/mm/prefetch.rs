...
impl AdaptivePrefetcher {
    ...
    pub fn new() -> Self {
        Self {
            ...
            evaluation_timer: crate::subsystems::time::timestamp_millis(),
    ...
    fn evaluate_strategy(&mut self) -> PrefetchStrategy {
        let current_time = crate::subsystems::time::timestamp_millis();
...
pub fn process_memory_access(addr: usize, page_size: usize, access_type: AccessType) {
    let access = MemoryAccessPattern {
        addr,
        page_size,
        timestamp: crate::subsystems::time::timestamp_millis(),
        access_type,
    };
