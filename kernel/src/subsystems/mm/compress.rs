...
pub static ENABLE_COMPRESSION: core::sync::atomic::AtomicBool =
...
pub struct CompressionStats {
    ...
    pub memory_pressure_triggered: AtomicUsize, /* Number of times memory pressure triggered
                                         * compression */
...
fn update_compression_stats(
    ...
    stats
        .total_compress_time_us
        .fetch_add(compress_time_us as usize, Ordering::Relaxed);
    ...
    let algo_stats = stats
        .per_algorithm_stats
        .entry(algorithm)
        .or_insert_with(AlgorithmStats::default);
    algo_stats.pages_compressed.fetch_add(1, Ordering::Relaxed);
    algo_stats
        .avg_compress_time_us
        .fetch_add(compress_time_us as usize, Ordering::Relaxed);
    ...
    if compressed_size < original_size {
        ...
        if count > 0 {
            ...
            algo_stats
                .compression_ratio
                .store(new_ratio, Ordering::Relaxed);
        } else {
...
fn update_decompression_stats(algorithm: CompressionAlgorithm, decompress_time_us: u64) {
    ...
    stats
        .total_decompress_time_us
        .fetch_add(decompress_time_us as usize, Ordering::Relaxed);
...
pub fn get_detailed_stats() -> CompressionStats {
    ...
    CompressionStats {
        ...
        total_compress_time_us: AtomicUsize::new(
            stats.total_compress_time_us.load(Ordering::Relaxed),
        ),
        total_decompress_time_us: AtomicUsize::new(
            stats.total_decompress_time_us.load(Ordering::Relaxed),
        ),
        memory_pressure_triggered: AtomicUsize::new(
            stats.memory_pressure_triggered.load(Ordering::Relaxed),
        ),
        adaptive_algorithm_changes: AtomicUsize::new(
            stats.adaptive_algorithm_changes.load(Ordering::Relaxed),
        ),
...
fn compress_high_ratio(src: &[u8], _level: u8) -> Vec<u8> {
    ...
    let mut sorted_bytes: Vec<(usize, u8)> = freq
        .iter()
        .enumerate()
        .map(|(i, &f)| (f, i as u8))
        .collect();
