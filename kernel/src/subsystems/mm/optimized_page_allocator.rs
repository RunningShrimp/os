...
/// Per-CPU page cache with bitmap-based tracking
///
/// This implementation uses fixed-size arrays instead of BTreeMap for better
/// cache locality and O(1) access time.
#[derive(Debug)]
pub struct PerCpuPageCache {
    /// Single page cache
    single_pages: Vec<PageFrame>,
    /// Multi-page cache by order (replaced BTreeMap with fixed array)
    /// Index is the order, value is the list of page frames
    multi_pages: [Vec<PageFrame>; (MAX_ORDER + 1) as usize],
    /// Bitmap to track which orders have cached pages (for fast lookup)
    /// Bit i is set if multi_pages[i] is non-empty
    orders_bitmap: AtomicUsize,
    /// Maximum cache size
...
impl PerCpuPageCache {
    ...
    pub fn new(max_cache_size: usize) -> Self {
        Self {
            ...
            multi_pages: [(); (MAX_ORDER + 1) as usize].map(|_| Vec::new()),
            orders_bitmap: AtomicUsize::new(0),
            max_cache_size,
    ...
    ///
    /// Uses bitmap for O(1) lookup instead of BTreeMap's O(log n)
    pub fn get_multi_pages(&mut self, order: PageOrder) -> Option<PageFrame> {
        ...
        let bitmap = self.orders_bitmap.load(Ordering::Acquire);

        // Fast check: if bitmap bit is not set, no pages cached for this order
        if (bitmap & (1usize << order_idx)) == 0 {
            self.cache_misses.fetch_add(1, Ordering::Relaxed);
            return None;
        }

        // Get pages from the array (O(1) access)
        if let Some(pfn) = self.multi_pages[order_idx].pop() {
            let page_count = 1usize << order;
            self.current_cache_size
                .fetch_sub(page_count, Ordering::Relaxed);

            // Update bitmap if this was the last page for this order
            if self.multi_pages[order_idx].is_empty() {
                self.orders_bitmap
                    .fetch_and(!(1usize << order_idx), Ordering::Release);
            }
        ...
        // Bitmap was set but list was empty (race condition or corruption)
        self.orders_bitmap
            .fetch_and(!(1usize << order_idx), Ordering::Release);
        self.cache_misses.fetch_add(1, Ordering::Relaxed);
        None
    ...
    ///
    /// Uses array indexing for O(1) insertion instead of BTreeMap's O(log n)
    pub fn put_multi_pages(&mut self, pfn: PageFrame, order: PageOrder) {
        ...
        let order_idx = order as usize;
        let was_empty = self.multi_pages[order_idx].is_empty();

        // Add to array (O(1) access)
        self.multi_pages[order_idx].push(pfn);
        self.current_cache_size
            .fetch_add(page_count, Ordering::Relaxed);

        // Update bitmap if this is the first page for this order
        if was_empty {
            self.orders_bitmap
                .fetch_or(1usize << order_idx, Ordering::Release);
        }
    }

    /// Flush the cache (return all pages to the main allocator)
    pub fn flush(&mut self) -> (Vec<PageFrame>, Vec<(PageOrder, Vec<PageFrame>)>) {
        let single_pages = core::mem::take(&mut self.single_pages);

        // Convert array to vector of tuples for compatibility
        let mut multi_pages = Vec::new();
        for (order, pages) in self.multi_pages.iter_mut().enumerate() {
            if !pages.is_empty() {
                multi_pages.push((order as PageOrder, core::mem::take(pages)));
            }
        }

        self.current_cache_size.store(0, Ordering::Relaxed);
        self.orders_bitmap.store(0, Ordering::Relaxed);
...
impl BuddyAllocator {
    ...
    fn add_to_free_list(&self, pfn: PageFrame, order: PageOrder) {
        ...
        unsafe {
            (*node).next =
                self.free_lists[order as usize].load(Ordering::Relaxed) as *mut FreeListNode;
            (*node).prev = null_mut();
...
impl OptimizedPageAllocator {
    ...
    pub fn defragment(&self) -> usize {
        ...
        for cache in &self.per_cpu_caches {
            ...
            let (single_pages, multi_pages_vec) = cache_guard.flush();
            ...
            for (order, pages) in multi_pages_vec {
                for pfn in pages {
