...
impl MemoryStatsCollector {
    ...
    pub fn new() -> Self {
        Self { stats: Mutex::new(MemoryManagementStats::default()) }
    }
    ...
    pub fn record_allocation(&self, mem_type: super::MemoryType, size: usize) {
        ...
        if stats.allocation_stats.current_allocated_bytes
            > stats.allocation_stats.peak_allocated_bytes
        {
            stats.allocation_stats.peak_allocated_bytes =
                stats.allocation_stats.current_allocated_bytes;
        }
...
pub fn get_stats_collector() -> &'static MemoryStatsCollector {
    unsafe { GLOBAL_STATS.as_ref().unwrap() }
}
