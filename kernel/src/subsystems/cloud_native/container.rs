...
impl Container {
    ...
    pub fn start(&mut self) -> Result<u32, i32> {
        ...
        // 使用OCI运行时创建容器
        let container_id = crate::subsystems::cloud_native::oci::create_oci_container(oci_spec)?;

        // 启动OCI容器
        let pid = crate::subsystems::cloud_native::oci::start_oci_container(&container_id)?;
    ...
    pub fn stop(&mut self, timeout_sec: Option<u32>) -> Result<(), i32> {
        ...
        if let Some(ref oci_spec) = self.oci_spec {
            ...
            crate::subsystems::cloud_native::oci::stop_oci_container(&oci_spec.id, timeout)?;
        }
    ...
    pub fn remove(mut self) -> Result<(), i32> {
        ...
        if let Some(ref oci_spec) = self.oci_spec {
            // 使用OCI运行时删除容器
            crate::subsystems::cloud_native::oci::delete_oci_container(&oci_spec.id)?;
        }
    ...
    fn create_oci_resources(&self) -> Result<Option<crate::subsystems::cloud_native::oci::OciLinuxResources>, i32> {
        ...
        if let Some(limit) = self.config.resources.memory_limit {
            memory = Some(crate::subsystems::cloud_native::oci::OciLinuxMemory {
                limit: Some(limit),
                reservation: None,
                swap: None,
                kernel: None,
        ...
        if memory.is_some() || cpu.is_some() {
            Ok(Some(crate::subsystems::cloud_native::oci::OciLinuxResources {
                memory,
                cpu,
                devices: Vec::new(),
                network: None,
    ...
    fn create_oci_linux_config(&self) -> Result<crate::subsystems::cloud_native::oci::OciLinux, i32> {
        ...
        // 添加标准命名空间
        namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
            typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::PID,
            path: None,
        });
        namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
            typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::Mount,
            path: None,
        });
        namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
            typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::UTS,
            path: None,
        });
        namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
            typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::IPC,
            path: None,
        });
        ...
        if self.config.network.mode != NetworkMode::Host {
            namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
                typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::Network,
                path: None,
            });
        }
        ...
        if self.config.security.user_ns.is_some() {
            namespaces.push(crate::subsystems::cloud_native::oci::OciLinuxNamespace {
                typ: crate::subsystems::cloud_native::oci::OciLinuxNamespaceType::User,
                path: None,
            });
        }

        Ok(crate::subsystems::cloud_native::oci::OciLinux {
    ...
    fn apply_resource_limits(&self) -> Result<(), i32> {
        // 内存限制
        if let Some(limit) = self.config.resources.memory_limit {
            crate::subsystems::cloud_native::cgroups::set_memory_limit_for_process(
        ...
        if let Some(limit) = self.config.resources.cpu_limit {
            crate::subsystems::cloud_native::cgroups::set_cpu_limit_for_process(
                self.pid.unwrap_or(0),
                limit,
            )?;
        }
        ...
        if let Some(limit) = self.config.resources.disk_limit {
            crate::subsystems::cloud_native::cgroups::set_disk_limit_for_process(
                self.pid.unwrap_or(0),
                limit,
            )?;
        }
