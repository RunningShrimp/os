...
/// 将进程添加到指定的 cgroup
pub fn add_process_to_cgroup(cgroup_name: &str, pid: u32) -> Result<(), i32> {
    let manager = get_v1_cgroup_manager_mut().ok_or(EIO)?;
    if let Some(cgroup) = manager.get_cgroup(cgroup_name) {
        let cg = cgroup.lock();
        cg.add_process(pid)?;
        Ok(())
    } else {
...
/// 从 cgroup 移除进程
pub fn remove_process_from_cgroup(cgroup_name: &str, pid: u32) -> Result<(), i32> {
    let manager = get_v1_cgroup_manager_mut().ok_or(EIO)?;
    if let Some(cgroup) = manager.get_cgroup(cgroup_name) {
        let cg = cgroup.lock();
...
/// 检查进程的 CPU 限制（返回配额和周期，单位：微秒）
/// 返回值：(quota, period)，如果 quota 为 -1 表示无限制
...
pub fn get_process_memory_limit(pid: u32) -> Option<u64> {
    ...
    let cgroup = manager.get_cgroup(cgroup_name)?;
    let cg = cgroup.lock();
    if let Some(ref memory_config) = cg.config.memory {
        memory_config.limit
    } else {
        None
    }
...
/// 检查进程是否超过内存限制
pub fn check_memory_limit(pid: u32, current_usage: u64) -> bool {
    if let Some(limit) = get_process_memory_limit(pid) {
        current_usage > limit
    } else {
        false // No limit means unlimited
    }
}
