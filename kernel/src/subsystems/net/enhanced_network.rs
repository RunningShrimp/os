...
impl EnhancedNetworkManager {
    ...
    pub fn socket(
        ...
        crate::println!(
            "[network] Created socket {}: family={:?}, type={:?}, protocol={:?}",
            socket_id,
            family,
            socket_type,
            protocol
    ...
    pub fn accept(&self, socket_id: usize) -> Result<(usize, SocketAddress), NetworkError> {
        ...
        if let Some(socket) = sockets.get(&socket_id) {
            ...
            crate::println!(
                "[network] Accepted connection on socket {}: new socket {} from {:?}",
                socket_id,
                new_socket_id,
                remote_address
            );
