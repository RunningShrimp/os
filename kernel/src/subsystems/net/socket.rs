...
impl TcpSocketWrapper {
    ...
    /// Receive data
    pub fn recv(&mut self, _buf: &mut [u8]) -> Result<usize, SocketError> {
        if self.state != TcpState::Established {
            return Err(SocketError::NotConnected);
        }
...
impl UdpSocketWrapper {
    ...
    pub fn send_to(&mut self, data: &[u8], dest: SocketAddr) -> Result<usize, SocketError> {
        ...
        if let Some(_dest_ip) = dest.ipv4_addr() {
    ...
    /// Receive data
    pub fn recv_from(&mut self, _buf: &mut [u8]) -> Result<(usize, SocketAddr), SocketError> {
        if self.state != UdpSocketState::Bound {
            return Err(SocketError::NotBound);
        }
