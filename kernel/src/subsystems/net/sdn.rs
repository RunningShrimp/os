...
impl FlowTable {
    ...
    pub fn lookup(&self, packet: &Packet) -> Option<FlowEntry> {
...
impl OpenFlowSwitch {
    ...
    /// Get switch features
    pub fn get_features(&self) -> &SwitchFeatures {
        &self.features
    }
    ...
    pub fn process_packet(&self, packet: Packet) -> Result<Vec<FlowAction>, SdnError> {
    ...
    /// Execute flow action
    fn execute_action(&self, packet: &Packet, action: &FlowAction) -> Result<(), SdnError> {
        match action {
            FlowAction::Output => {
    ...
    fn send_to_controller(&self, packet: Packet) {
