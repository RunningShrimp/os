...
fn generate_syscall_matrix() -> Result<()> {
    ...
    let match_re = Regex::new(r"(?m)^\s*match\s+syscall_num\s*\{([\s\S]*?)\}")?;
    let arm_re = Regex::new(r"(?m)\s*SysNum::([A-Za-z_][A-Za-z0-9_]*)\s*=>\s*(sys_[a-z0-9_]+)\(")?;

    let mut variants: Vec<(String, usize)> = Vec::new();
    if let Some(cap) = enum_re.captures(&mod_content) {
        let body = &cap[1];
    ...
    if let Some(cap) = match_re.captures(&mod_content) {
        let body = &cap[1];
    ...
    // 系统调用模块已完全迁移到syscalls/mod.rs，无需检查常量差异
